...
- ~~**`utm_*`/`gclid`/`fbclid` продовжуємо тягнути** в href пагінації~~ — **скасовано після ревю.** Дзеркалення всього query string роздувало документ (junk на 16 KB → +145 KB розмітки) і перезапускало GA4-атрибуцію на кожному кліку пагінації. `buildPageHref` тепер приймає явний санітизований набір (`page`, `sort`, `minPrice`, `maxPrice`, `options`, `q`); tracking-параметри у внутрішні посилання не потрапляють.
